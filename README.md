...
This Docker image is built on [Chainguard's Wolfi](https://www.chainguard.dev/unchained/introducing-wolfi-the-first-linux-un-distro), a security-focused, minimal Linux distribution designed for contai[...]
...
```bash
docker run -e DERP_DOMAIN=derper.your-domain.com -e DERP_HOME= -p 80:80 -p 443:443 -p 3478:3478/udp rgv151/derper
```

| env                    | required | description                                                                                                                                    | default value     |
| -------------------    | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| DERP_DOMAIN            | true     | derper server hostname                                                                                                                         | your-hostname.com |
| DERP_HOME              | false    | what to serve at the root path. Leave empty (the default) for the default homepage, set to `blank` for a blank page, or set to a URL to redirect | ""                |
...
services:
  derper:
    ...
    environment:
      ...
      - DERP_HOME=            # leave empty for default homepage; use "blank" for a blank page, or a URL to redirect (e.g. https://example.com)
...
In order to use `DERP_VERIFY_CLIENTS`, the container needs access to Tailscale's Local API, which can usually be accessed through `/var/run/tailscale/tailscaled.sock`. If you're running Tailscale bare[...]
